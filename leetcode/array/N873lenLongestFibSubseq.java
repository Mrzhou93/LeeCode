...
public class N873lenLongestFibSubseq {
    public int lenLongestFibSubseq(int[] A) {
        ...
        for (int i = 0; i < len; i++){
            for (int j = 0; j < i; j++){
                ...
                if (k >= 0 && k < j){
                    int tmp = longest.getOrDefault(k * len + j, 2) + 1;
                    longest.put(j * len + i, tmp);
    ...
    public static void main(String[] args) {
        N873lenLongestFibSubseq s = new N873lenLongestFibSubseq();

        System.out.println(s.lenLongestFibSubseq(new int[]{1, 2, 3}));
