...
public class N90subsetsWithDup {
    public List<List<Integer>> subsetsWithDup(int[] nums) {
        ...
        backRrace(res, new ArrayList<>(), nums, visited, 0);

        return res;
    }

    private void backRrace(List<List<Integer>> res, List<Integer> partRes, int[] nums, boolean[] visited, int index){
        ...
        for (int i = index; i < nums.length; i++){
            ...
            if (!visited[i]){
                list.add(nums[i]);
                visited[i] = true;
                backRrace(res, list, nums, visited, i + 1);
                visited[i] = false;
            }
        }
    }
